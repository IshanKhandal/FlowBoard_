...
![FlowBoard board](./screenshot.png)
>>>>>>> a8abb26 (Improve README with screenshot, badges, and tightened wording)
...
### Drag a card. Watch it move on someone else is screen, instantly.
...
**Live:** https://flowboard-blond-two.vercel.app
*(Backend runs on Render is free tier - the first load after inactivity can take 30-60 seconds to wake up.)*
>>>>>>> a8abb26 (Improve README with screenshot, badges, and tightened wording)
...
- Online presence, so you know who is here right now
...
**One repo, not two.** Client and server are small enough and tightly coupled enough in purpose that splitting them just means two READMEs for one system. The trade-off: no independent deploy isolation, which matters more at a scale this project is not at.
...
**Normalized state.** Tasks and columns live as flat objects linked by ID, not nested. Moving a task between columns becomes "update two arrays of IDs" - and it is the same shape that makes broadcasting small diffs over a socket actually tractable.

**dnd-kit over react-dnd.** Keyboard and touch support out of the box, which matters for both accessibility and mobile.

**WebSockets, not polling.** A task move or a cursor position has to feel instant. Polling adds latency a collaborative board cannot afford.
